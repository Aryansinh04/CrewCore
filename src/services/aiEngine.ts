...
const fetchAiEndpoint = async (path: string, body: any) => {
...
export const aiEngine = {
  ...
  generateJobDescription: async (
    ...
    try {
      const data = await fetchAiEndpoint('/api/ai/generate-jd', { title, domain, keywords });
      return data.jd;
    } catch (err: any) {
      console.warn('Backend AI offline, falling back to local JD generator:', err.message);
...
- Flexible remote/hybrid working arrangement.`;
  ...
  screenCandidateApplication: async (
    ...
    try {
      return await fetchAiEndpoint('/api/ai/screen', { resume, answers });
    } catch (err: any) {
      console.warn('Backend AI offline, falling back to local Screening assessor:', err.message);
  ...
  generateInterviewQuestions: async (domain: string, seniority: string): Promise<string[]> => {
    try {
      return await fetchAiEndpoint('/api/ai/interview-questions', { domain, seniority });
    } catch (err: any) {
      console.warn('Backend AI offline, falling back to local interview templates:', err.message);
      const questionsMap: { [key: string]: string[] } = {
        ...
        'hr-operations': [
          'How do you analyze pay compensation and market benchmarks for standardizing salaries?',
  ...
  chatWithBot: async (
    ...
    try {
      const data = await fetchAiEndpoint('/api/ai/chat', { history, message, roleMode });
      return data.response;
    } catch (err: any) {
      console.warn('Backend AI chat offline, falling back to local keyword responder:', err.message);
